...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        if (S[i] == (char)('0' + now)) // 今見ている数とS[i]が違う場合
        {
            cnt++;
        }
        else
    ...
    for (int i = 0; i < (int)Nums.size(); i += 2)
    {
        ...
        int left = i;
        int right = min(i + Add, (int)Nums.size()); // 配列長を超えないように押さえつける
        int tmp = sum[right] - sum[left];           //[left,right)は、leftを0から始めることで累積和ではsum[right]-sum[left]
        // 累積和では1-indexedになっており、right(1-indexed)=right-1(0-indexed)
        ans = max(tmp, ans);
    }
    cout << ans << endl;
    return 0;
