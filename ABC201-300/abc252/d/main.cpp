...
int main()
{
    ...
    cin.tie(nullptr);
    // 下記をC++に書き換えただけ。解説AC
    // https://atcoder.jp/contests/abc252/submissions/31635907
    // 学び：問題文の条件はすべての要素が異なることしか表していない。例えば値の大きさを基準に順序を入れ替えて良い
    // 学び：最大値が小さい(今回だと2*10**5)場合は、個数管理には辞書よりもvectorののほうが扱いやすい
    ll N;
    cin >> N;
    vector<ll> A(N);
    rep(i, N)
    ...
    rep(j, N)
    {
        // i(jより値が小さい要素のうちどれか)*k(jより値の大きい要素のうちどれか)
